export function setupGenerateReport(): void {
  ...
  if (!generateBtn || !checklist || !draftPreview || !draftBodyContent) return;

  const items = checklist.querySelectorAll<HTMLLabelElement>(".report-item");
  ...
  const updateDraft = () => {
    ...
    items.forEach((item) => {
      ...
      if (checkbox.checked) selected.push(item.dataset.text || "");
    });
...
export function setupScrollLinks(): void {
  document.querySelectorAll<HTMLElement>("[data-scroll-to]").forEach((e1) => {
...
export function setupRobotEyes(): void {
  const wrap = document.getElementById("robot-wrap");
  const pupilLeft = document.getElementById("pupil-left");
  const pupilRight = document.getElementById("pupil-right");
  if (!wrap || !pupilLeft || !pupilRight) return;
